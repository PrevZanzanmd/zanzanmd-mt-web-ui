...
import { handleFullDate, throwError } from '../../fetchApi/commonApi'
...
function* getCashierlist(){
	yield takeLatest(ACTION.GET_CASHIERLIST, function* (p){
		...
		data.code == '200' ? yield put({type: ACTION.GET_CASHIERLIST_COMPLETE, data: data.data}) : (
			data.code == '60012' ? yield put({type: ACTION.GET_CASHIERLIST_COMPLETE, data: [] }) : null,
			throwError(data)
		)
